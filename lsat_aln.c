...
void rearr_aln_res(aln_res *res, int n)
{
    ...
    for (a_i = 0; a_i < n; ++a_i) {
        p = res+a_i;
        for (i = 0; i < p->l_n; ++i) {
            if (p->la[i].merg_msg.x == 0) continue;
            if (tri_n == tri_m) {
    ...
    for (i = 1; i < tri_n; ++i) {
        if (!get_cover_res(reg, res, i, &cov_tri_i, tri, head, head_n)) { // NOT cover
            ...
            (res+tri[cov_tri_i].x)->la[tri[cov_tri_i].y].merg_msg.y = 1;
...
void _aln_res_output(FILE *outp, aln_res *res, int res_n, char *name, uint8_t *seq, char *qual, bntseq_t *bns)
{
    int i, j, n, l, all=0;
    kstring_t sam_str; memset(&sam_str, 0, sizeof(kstring_t));
            ...
            for (j = 0; j <= p->la[i].cur_res_n; ++j) {
                ...
                if (j == 0) {
                    ...
                    for (l = 0; l < p->la[i].XA_n; ++l) {
                        ...
                        ksprintf(&XA_str, "%d,%c%lld,", XA_res->chr,"-+"[XA_res->nsrand], (long long)XA_res->offset); 
